...
result_path = 'result/dense result.csv'
...
for model in model_list:
    ...
    model_path = 'dense_network/' + str(model) + '.rds'
    dag_true = base.readRDS(model_path)
    cpdag_true = bnlearn.cpdag(dag_true)
    data_path = 'dense data/' + str(model) + '/' + str(model) + '_Clean.csv'
    if path.isfile(data_path):
        ...
        os.makedirs('dense data/' + str(model))
    ...
    for noise in noise_list:
        ...
        data_path = 'dense data/' + str(model) + '/' + str(model) + '_' + noise + '.csv'
        if path.isfile(data_path):
